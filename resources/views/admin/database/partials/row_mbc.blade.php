...
<tr data-id="{{ $item->id }}" style="background-color: #ffffff; color: #212529; transition: background-color 0.3s ease;">
    ...
    @if($userRole === 'cs-mbc')
    {{-- Potensi Ikut Kelas Column for CS-MBC --}}
    <td class="text-center" style="vertical-align: middle;">
        @php
            ...
            // Filter available classes scheduled after today
            $today = \Carbon\Carbon::now()->startOfDay();
            $availableClasses = $kelas->filter(function($k) use ($today) {
        ...
        <div class="d-flex flex-column align-items-center justify-content-center p-2" style="gap: 4px;">
            @if($validSalesplans->isNotEmpty())
                @foreach($validSalesplans as $sp)
                    ...
                    <div class="d-flex align-items-center justify-content-center mb-1" style="gap: 8px;">
                        ...
                        <!-- Status Dropdown for this specific class prospect -->
                        <select class="form-control form-control-sm inline-status-select shadow-sm" 
                                data-data-id="{{ $item->id }}" 
                                data-kelas-id="{{ $sp->kelas_id }}"
                                data-kelas-nama="{{ $sp->kelas?->nama_kelas ?? '' }}"
                                style="width: 125px; font-size: 0.75rem; font-weight: bold; border-radius: 8px; padding: 2px 6px; height: auto; cursor: pointer; background-color: {{ $cfg['bg'] }}; color: {{ $cfg['text'] }}; border: 1px solid #ccc;"
                                onchange="updateInlineStatusFromTable(this)">
                            <option value="cold" {{ $statusKeySP === 'cold' ? 'selected' : '' }} style="background:#ffffff; color:#6c757d;">Cold</option>
                            <option value="tertarik" {{ $statusKeySP === 'tertarik' ? 'selected' : '' }} style="background:#F2F527; color:#000000;">Tertarik</option>
                            <option value="mau_transfer" {{ $statusKeySP === 'mau_transfer' ? 'selected' : '' }} style="background:#3CDE1D; color:#000000;">Mau Transfer</option>
                            <option value="sudah_transfer" {{ $statusKeySP === 'sudah_transfer' ? 'selected' : '' }} style="background:#1786E6; color:#ffffff;">Sudah Transfer</option>
                        </select>
                        ...
                        <button type="button" class="btn btn-sm btn-outline-danger p-0"
                                ...
                                onclick="deleteProspectFromTable(this, {{ $item->id }}, {{ $sp->kelas_id }})"
            ...
            <div class="inline-add-class-wrapper mt-1">
                <!-- Dropdown (hidden by default, shown when clicking + button) -->
                <select class="form-control form-control-sm mb-1 inline-add-class-select d-none shadow-sm" 
                        style="width: 160px; font-size: 0.75rem; font-weight: bold; border-radius: 8px; border: 1.5px solid #28a745; height: 32px;"
                        onchange="saveNewProspectFromTable(this, {{ $item->id }})">
                    <option value="">-- Pilih Kelas Baru --</option>
                    @foreach($availableClasses as $k)
                        <option value="{{ $k->id }}">{{ str_contains($k->nama_kelas, 'Muslim Indonesia') ? 'M1T' : $k->nama_kelas }}</option>
                    @endforeach
                </select>
                ...
                <button type="button" class="btn btn-sm btn-success rounded-circle shadow-sm" 
                        ...
                        onclick="showInlineAddClassDropdown(this)"
                        title="Tambah Prospek Kelas Baru">
