    ...
    <tr>
        ...
        <td class="p-1 align-middle">
            @php
                ...
                $isAdmin = (in_array($userRole, ['administrator', 'pusat', 'admin']) || auth()->user()->hasSubrole('spp_admin') || auth()->user()->name === 'Linda');
             @endphp
            ...
            <td class="text-center align-middle p-0 spp-col {{ $i > 1 ? 'spp-extra' : '' }}">
                ...
                @if($isVisible)
                    @php
                        ...
                        if (request('filter_spp_status') === 'menunggak' && $item->status === 'Aktif' && !$isAllPaid && !$isPaid) {
                            $joinMonth = 1;
                            if ($item->tanggal_masuk) {
                                try {
                    ...
                    <div class="spp-wrapper {{ $isMenunggakThisMonth ? 'spp-menunggak-border' : '' }}">
                        ...
                        @php
                            $canCheckSpp = (auth()->user()->hasSubrole('spp_admin') || auth()->user()->name === 'Linda' || strtolower(auth()->user()->role) === 'administrator');
                        @endphp
